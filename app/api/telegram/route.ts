...
<head>
  ...
  <style>
    ...
    .header { background: linear-gradient(135deg, #0a0f1e 0%, #0d1a0d 100%); padding: 40px; text-align: center; }
    ...
    .step-num { width: 28px; height: 28px; min-width: 28px; background: #22c55e; color: #021a09; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 13px; }
  ...
  <div class="wrapper">
    ...
    <div class="body">
      ...
      <div class="highlight-box">
        <p>✅ <strong>Your enrollment is confirmed.</strong> I will reach out personally within the next 24 hours to get you set up and ready to go.</p>
      </div>
      <p>Here is what to expect:</p>
      <div class="steps">
        <div class="step">
          <div class="step-num">1</div>
          <div class="step-text"><strong>Welcome call</strong> — We will hop on a quick call to go over your goals and what you want to achieve.</div>
        </div>
        <div class="step">
    ...
    <div class="footer">
      <p>Mind Over Markets Mentorship<br/><a href="mailto:${FROM_EMAIL}">${FROM_EMAIL}</a></p>
    </div>
  </div>
</body>
</html>`
...
async function handleWelcome(chatId: number, args: string) {
  ...
  if (!leads || leads.length === 0) return sendTelegram(chatId, `❌ No lead found with email: \`${email}\``)

  const lead    = leads[0]
  const name    = lead.full_name || lead.name || 'there'
  const program = lead.program || 'MOM Mentorship'

  await sendTelegram(chatId, `⏳ Sending welcome email to *${name}*...`)
...
async function handleAll(chatId: number, statusFilter?: string) {
  let query = supabase
    .from('leads')
    .select('full_name, name, email, status, program, created_at')
    .order('created_at', { ascending: false })
    .limit(10)
